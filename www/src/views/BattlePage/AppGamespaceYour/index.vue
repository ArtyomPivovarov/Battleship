  ...
  <div class="game-space">
    ...
    <div class="info">
      ...
      <div class="turn-wrapper">
        <div v-show="myTurn" class="turn">
          <span>Сейчас право хода за тобой</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    shipCount(type) {
      ...
      for (let i = 1; i <= ships; i++) {
        ...
        const brokenDecks = this.fieldMy.reduce((sum, col) => {
          return (
  ...
  computed: {
    ...
    myTurn() {
      return this.game.myTurn && this.game.status != '3'
    },
    damage() {
      const decks = 20;
